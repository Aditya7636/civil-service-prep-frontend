...
import { fetchCatalogProfessions } from '../../lib/api';
import type { CatalogProfession } from '../../lib/types';
...
export default function ProfessionSelectorPage() {
  ...
  const handleProfessionSelect = (value: string) => setSelectedProfession(value);
  ...
  const selectedProfessionData = professions.find((item) => item.slug === selectedProfession);

  const categoryIcons: Record<string, any> = {
    'operational-delivery': Building2,
    policy: FileText,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10">
            <h2 className="text-2xl font-semibold text-[var(--navy-900)] mb-6">
          ...
          <div className="space-y-6 mb-8">
            <div
              className="border-2 border-[var(--navy-700)]/20 bg-gradient-to-br from-white via-white to-[var(--navy-50)] hover:border-[var(--navy-700)]/60 hover:shadow-2xl transition-all duration-300 cursor-pointer group relative overflow-hidden rounded-2xl"
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="flex-shrink-0">
                    ...
                    <p className="text-base text-[var(--navy-600)] leading-relaxed mb-4 max-w-md">
                      A comprehensive preparation programme covering all core Success Profile elements applicable
                      across the entire Civil Service. Ideal for candidates applying to multiple roles or exploring
                      different departments.
                    </p>
                    <button className="bg-[var(--navy-800)] hover:bg-[var(--navy-900)] text-white h-12 px-8 shadow-md hover:shadow-lg transition-all rounded-lg inline-flex items-center">
            ...
            <div className="border-2 border-[var(--navy-700)]/20 bg-gradient-to-br from-white via-white to-[var(--navy-50)] hover:border-[var(--navy-700)]/60 hover:shadow-2xl transition-all duration-300 relative overflow-hidden rounded-2xl">
              <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-[var(--navy-700)]/8 to-transparent rounded-bl-full"></div>
              <div className="absolute bottom-0 left-0 w-32 h-32 bg-gradient-to-tr from-[var(--navy-800)]/5 to-transparent rounded-tr-full"></div>
              <div className="p-6 sm:p-8 relative">
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="flex-shrink-0 md:w-80">
                    ...
                    <div className="space-y-3 mb-6">
                      ...
                      <select
                        ...
                        onChange={(event) => handleProfessionSelect(event.target.value)}
                        className="h-12 text-base border-2 border-[var(--sand-300)] focus:ring-[var(--navy-700)] focus:border-[var(--navy-700)] rounded-lg px-4 w-full"
                        disabled={loading}
                      >
                        <option value="">
                          {loading ? 'Loading professions...' : 'Choose from 30+ professions...'}
                        </option>
                    ...
                    <button
                      ...
                      className="w-full bg-[var(--navy-800)] hover:bg-[var(--navy-900)] text-white h-12 px-8 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center justify-center"
                    >
          ...
          {selectedProfessionData && (
            <div className="border-2 border-[var(--navy-700)] bg-gradient-to-br from-[var(--navy-900)] to-[var(--navy-800)] text-white rounded-2xl">
              <div className="p-6">
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0 w-12 h-12 bg-white/10 rounded-lg flex items-center justify-center">
                    {(() => {
                      const Icon = categoryIcons[selectedProfessionData.category.slug];
                  ...
                  <div className="flex-1">
                    ...
                    {!!selectedProfessionData.alignedBehaviours?.length && (
                      <div className="mb-3">
                        <p className="text-xs text-white/70 mb-2 uppercase tracking-wide">Aligned Behaviours</p>
                        <div className="flex flex-wrap gap-2">
                          {selectedProfessionData.alignedBehaviours.map((behaviour) => (
                            <span
                              key={behaviour.id}
                              className="px-2.5 py-1 rounded-full text-xs bg-white/15 text-white"
                            >
                              {behaviour.name}
                            </span>
                          ))}
                    ...
                    {!!selectedProfessionData.recommendedAssessments?.length && (
                      <div className="mb-3">
                        <p className="text-xs text-white/70 mb-2 uppercase tracking-wide">Recommended Assessments</p>
                        <div className="flex flex-wrap gap-2">
                          {selectedProfessionData.recommendedAssessments.map((method) => (
                            <span
                              key={method.id}
                              className="px-2.5 py-1 rounded-full text-xs bg-white/15 text-white"
                            >
                    ...
                    {selectedProfessionData.slug === 'digital-data' ? (
